const quiz = [
  {
    ...
    explanation: "投票所で用意した鉛筆を必ず使わなければいけない訳ではありません。ボールペン等はインクがにじむ可能性があるので、鉛筆かシャープペンシルの使用を推奨しています。感染症対策で、使い捨てできるクリップペンシルを導入する自治体もあります。",
  ...
  },
  {
    ...
    explanation: "使い終わった投票用紙は、プラスチック製品としてリサイクルされ、うちわの柄、投票所の床に敷く養生シート、ポスター掲示板などに生まれ変わっています。また、投票用紙は折り畳んでも投票箱の中で自然に開くので開票作業がスムーズに行えます。しっとり滑らかな書き心地をぜひ投票所でお試しください。",
  ...
  },
  {
    ...
    explanation: "公職選挙法の改正で平成28年から、選挙人が投票所に同伴できる子供の範囲が「幼児」から「18歳未満」に拡大されました。まだ18歳になっていなくても、早い段階から投票所の雰囲気に触れて、一票の大切さを感じ取ってみてください。 ただし、大声で騒いだり、他の人の投票をのぞき見したりはしないでくださいね。",
    explanation2: ["#18歳未満でも入れるよ", "#親と一緒に行ってみた", "#一票の大切さを学ぼう"]
  },
  {
    id: "4",
    question: '候補者を広くアピールする選挙カー。\n日本では、他にどんな乗り物で選挙運動をすることができる？',
    answers: {
      A: "船",
      B: "オープンカー",
      C: "セグウェイ"
...
const setUpQuiz = () => {
  ...
  if (window.location.pathname.includes('/quiz/question.html')) {
   ...
   if (quiz[quizIndex]===quiz.slice(-1)[0]) {
...
const clickHandler = (e) => {
  ...
  setTimeout(() => location.href = "./answer.html?quizIndex=" + quizIndex+"&score="+score, 1500);
}
  ...
  let count = 15;
    ...
    function timer() {
      if (count >= 0) {
        ...
        location.href = "./answer.html?quizIndex=" + quizIndex + "&score=" + score;
      }
    }
